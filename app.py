...
def load_data():
    ...
    # Bersihkan nama kolom (lowercase, strip)
...
def train_model(data):
    # Pilih fitur yang akan digunakan untuk prediksi harga
    features = ['model_year', 'odometer', 'condition', 'cylinders', 'fuel', 
    ...
    numeric_features = ['model_year', 'odometer', 'cylinders']
    categorical_features = ['condition', 'fuel', 'transmission', 'type', 'paint_color', 'is_4wd']
    ...
    # Pipeline dengan regresi linear
    ...
    # Kita sudah fit di semua data, tapi untuk evaluasi kita fit ulang di train set saja
...
# ==================== SIDEBAR FILTER (Data Eksplorasi) ====================
st.sidebar.header("🔍 Filter Data Eksplorasi")

# Filter berdasarkan model (merek/mobil)
model_list = ['Semua'] + sorted(df['model'].dropna().unique().tolist())
selected_model = st.sidebar.selectbox("Pilih Model", model_list)

# Filter berdasarkan tahun
year_min = int(df['model_year'].min())
year_max = int(df['model_year'].max())
selected_year = st.sidebar.slider("Rentang Tahun", year_min, year_max, (year_min, year_max))

# Filter berdasarkan harga
price_min = int(df['price'].min())
price_max = int(df['price'].max())
selected_price = st.sidebar.slider("Rentang Harga", price_min, price_max, (price_min, price_max))

# Filter berdasarkan kondisi
condition_list = ['Semua'] + sorted(df['condition'].dropna().unique().tolist())
selected_condition = st.sidebar.selectbox("Pilih Kondisi", condition_list)

# Filter berdasarkan odometer
odo_min = int(df['odometer'].min())
odo_max = int(df['odometer'].max())
selected_odo = st.sidebar.slider("Rentang Odometer (km)", odo_min, odo_max, (odo_min, odo_max))
...
if submitted:
    ...
    # Tentukan kelayakan (harga input <= 90% dari prediksi dianggap layak)
    threshold = 0.9
